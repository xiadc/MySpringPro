	...
	$(".jian").click(function(){
		...
		if(quantity == 1){
			if(confirm("你确定要删除该条目吗？")){
				location = "/ssmbook/batchDelete?cartItemIds="+id;
			}
		}else{
...
function sendAjaxQuantity(cartItemId,quantity){
	$.ajax({
		url:"/ssmbook/updateQuantity.action",
...
function batchDelete(){
	...
	location = "/ssmbook/batchDelete.action?cartItemIds="+cartItemIdArray;
}
...
<c:forEach items="${list}" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/load.action?bid=${cartItem.book.bid }'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.book.image_b }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='/load.action?bid=${cartItem.book.bid }'/>"><span>${cartItem.book.bname }</span></a>
		</td>
		...
		<td>
			<a href="<c:url value='/batchDelete.action?cartItemIds=${cartItem.cartItemId}'/>">删除</a>
		</td>
	</tr>
</c:forEach>
	...
	<form id="jieSuanForm" action="<c:url value='/loadCartItems.action'/>" method="post">
